...
if __name__ == '__main__':
    ...
    # schema = ['TAG', 'TEXT']
    # # 设定抽取目标和定制化模型权重路径
    # my_ie = UIEPredictor(model='uie-base', task_path='D:\\PythonProject1\\PythonProject\\uie_pytorch\\check_point\\model_best', schema=schema)
    # pprint(my_ie("09_尚硅谷_zk_集群_安装"))

    schema = ['TAG']
    # 设定抽取目标和定制化模型权重路径
    my_ie = UIEPredictor(model='uie-base', task_path='D:\\PythonProject1\\PythonProject\\smart_edu\\uie_pytorch\\check_point_course\\model_best', schema=schema)
    pprint(my_ie(['下列选项中不能进行自动类型提升的是()','<p>在进行 Java 代码编译和运行的过程中，哪种格式的文件可以被编译成字节码文件（）</p>','在进行 Java 代码编译和运行的过程中，哪种格式的文件可以被编译成字节码文件（）',"03_ClickHouse入门_介绍&特点",'08-尚硅谷-Flume官网案例-案例测试.avi','12-尚硅谷-Flume动态监控多目录多文件-需求说明&配置信息.avi','深入解析SpringMVC框架，涵盖控制器、视图解析器、数据绑定、拦截器、文件上传等功能，构建Web应用的核心技术。']))
